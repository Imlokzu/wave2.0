...
addActionHandler('devSkipLogin', (global): ActionReturnType => {
  if (!DEBUG) {
    return global;
  }
  ...
  const currentUserId = /^\d+$/.test(global.currentUserId || '')
...
addActionHandler('completeWaveAuth', (global, actions, payload): ActionReturnType => {
  const tabId = getCurrentTabId();
